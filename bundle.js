...
class Banner {
  ...
  constructor(container_id) {
    xml("./modules/Banner/banner.svg").mimeType("image/svg+xml").get((error, xml) => {
      ...
      function myTrans() {
        spaceInvaders.transition().ease(linear$1).duration(2000).style("transform", "translate(0px, -2.655306px)")
          .on("end", function() {
            select(this).transition().ease(linear$1).duration(2000).style("transform", "translate(20px, -2.655306px)")
              .on("end", function() {
                myTrans();
              });
          });
...
=======
    ...
    self.width = container_width - self.padding.right - self.padding.left;
    self.height = 470;
>>>>>>> 62edcdd698e9d7866e085bdfcbf9f32b9b79703d
...
<<<<<<< HEAD
    ...
    let oScale = linear$2()
                    .domain([0, max(self.data, function(game) {
                      return game.Global_Sales;
                    })])
                    .range([0, 1])
                    .clamp(true);
